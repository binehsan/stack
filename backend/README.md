...
The only cap left anywhere in the API: `GROUP_MEMBER_ABUSE_CAP` (20, in
`family/models.py`), enforced in `family/views.py`'s
`RespondGroupInviteView`. Not a paywall — it applies to every group
regardless of who founded or joined it, and returns
`400 {"code": "GROUP_MEMBER_CAP"}` with plain "this group is full" wording,
purely to stop a group stack being abused as an unlimited public broadcast
list. The app has no paid tier, no purchase/subscription flow, and no other
usage limits anywhere.
