// Ported 1:1 from frontend/src/theme.js (the mobile app's design tokens) so
// the website reads as the same product, not a reskin. Keep these two files
// in sync if the mobile palette ever changes.
export const themes = {
  dawn: {
    label: 'Dawn',
    gradient: ['#FFEB97', '#E7B768', '#583714'],
    card: 'rgba(255, 250, 235, 0.82)',
    cardBorder: 'rgba(88, 55, 20, 0.14)',
    cardElevated: 'rgba(255, 253, 247, 0.98)',
    text: '#3A2413',
    textMuted: '#6B4A28',
    accent: '#A8551C',
    accentSoft: 'rgba(168, 85, 28, 0.12)',
    onAccent: '#FFFFFF',
    danger: '#D64545',
    success: '#3E8F5D',
    shadow: '#2A1808',
    overlay: 'rgba(58, 36, 19, 0.32)',
  },
  dusk: {
    label: 'Dusk',
    gradient: ['#583714', '#3B2410', '#1C1108'],
    card: 'rgba(255, 235, 151, 0.07)',
    cardBorder: 'rgba(255, 235, 151, 0.16)',
    cardElevated: 'rgba(43, 26, 12, 0.97)',
    text: '#FBEFD9',
    textMuted: '#D8B98C',
    accent: '#FFC15E',
    accentSoft: 'rgba(255, 193, 94, 0.18)',
    onAccent: '#2B1A0C',
    danger: '#E5484D',
    success: '#6FDB93',
    shadow: '#000000',
    overlay: 'rgba(10, 6, 3, 0.5)',
  },
  purpleDawn: {
    label: 'Premium Purple',
    gradient: ['#DCB8FF', '#A868D9', '#3D1F5C'],
    card: 'rgba(250, 244, 255, 0.82)',
    cardBorder: 'rgba(74, 42, 107, 0.14)',
    cardElevated: 'rgba(253, 250, 255, 0.98)',
    text: '#3A1F52',
    textMuted: '#6B4C87',
    accent: '#7C3AED',
    accentSoft: 'rgba(124, 58, 237, 0.12)',
    onAccent: '#FFFFFF',
    danger: '#D64545',
    success: '#3E8F5D',
    shadow: '#2A1030',
    overlay: 'rgba(42, 16, 48, 0.32)',
  },
  purpleDusk: {
    label: 'Premium Purple',
    gradient: ['#4C2A72', '#2E1747', '#140A1F'],
    card: 'rgba(233, 213, 255, 0.07)',
    cardBorder: 'rgba(233, 213, 255, 0.16)',
    cardElevated: 'rgba(36, 20, 50, 0.97)',
    text: '#F0E3FF',
    textMuted: '#C9A8E0',
    accent: '#B980F0',
    accentSoft: 'rgba(185, 128, 240, 0.18)',
    onAccent: '#241033',
    danger: '#E5484D',
    success: '#6FDB93',
    shadow: '#000000',
    overlay: 'rgba(10, 5, 15, 0.5)',
  },
  forestDawn: {
    label: 'Forest Green',
    gradient: ['#C9E6CE', '#7CB893', '#204D34'],
    card: 'rgba(240, 250, 242, 0.82)',
    cardBorder: 'rgba(31, 107, 69, 0.14)',
    cardElevated: 'rgba(247, 253, 249, 0.98)',
    text: '#153826',
    textMuted: '#3F6B52',
    accent: '#1F6B45',
    accentSoft: 'rgba(31, 107, 69, 0.12)',
    onAccent: '#FFFFFF',
    danger: '#D64545',
    success: '#3E8F5D',
    shadow: '#0C2417',
    overlay: 'rgba(12, 36, 23, 0.32)',
  },
  forestDusk: {
    label: 'Forest Green',
    gradient: ['#204D34', '#123322', '#081810'],
    card: 'rgba(180, 230, 197, 0.07)',
    cardBorder: 'rgba(180, 230, 197, 0.16)',
    cardElevated: 'rgba(15, 38, 26, 0.97)',
    text: '#E3F5E7',
    textMuted: '#A8CDB4',
    accent: '#4FCB8A',
    accentSoft: 'rgba(79, 203, 138, 0.18)',
    onAccent: '#0C2417',
    danger: '#E5484D',
    success: '#6FDB93',
    shadow: '#000000',
    overlay: 'rgba(4, 12, 8, 0.5)',
  },
  alpineDawn: {
    label: 'Alpine Blue',
    gradient: ['#C7E3FA', '#5F9EDB', '#173A5E'],
    card: 'rgba(240, 248, 255, 0.82)',
    cardBorder: 'rgba(23, 58, 94, 0.14)',
    cardElevated: 'rgba(247, 251, 255, 0.98)',
    text: '#122A44',
    textMuted: '#3D5C7A',
    accent: '#1C5F9E',
    accentSoft: 'rgba(28, 95, 158, 0.12)',
    onAccent: '#FFFFFF',
    danger: '#D64545',
    success: '#3E8F5D',
    shadow: '#0A1E33',
    overlay: 'rgba(10, 30, 51, 0.32)',
  },
  alpineDusk: {
    label: 'Alpine Blue',
    gradient: ['#173A5E', '#0E2338', '#050D16'],
    card: 'rgba(180, 215, 245, 0.07)',
    cardBorder: 'rgba(180, 215, 245, 0.16)',
    cardElevated: 'rgba(14, 35, 56, 0.97)',
    text: '#E4F1FC',
    textMuted: '#A8C6DE',
    accent: '#5FB0F0',
    accentSoft: 'rgba(95, 176, 240, 0.18)',
    onAccent: '#0E2338',
    danger: '#E5484D',
    success: '#6FDB93',
    shadow: '#000000',
    overlay: 'rgba(5, 10, 16, 0.5)',
  },
};

export const themeFamilies = [
  { id: 'classic', label: 'Classic', pro: false, light: 'dawn', dark: 'dusk' },
  { id: 'purple', label: 'Premium Purple', pro: true, light: 'purpleDawn', dark: 'purpleDusk' },
  { id: 'forest', label: 'Forest Green', pro: true, light: 'forestDawn', dark: 'forestDusk' },
  { id: 'alpine', label: 'Alpine Blue', pro: true, light: 'alpineDawn', dark: 'alpineDusk' },
];

export const radii = { sm: 12, md: 18, lg: 24, pill: 999 };
export const spacing = { xs: 4, sm: 8, md: 16, lg: 24, xl: 32 };
