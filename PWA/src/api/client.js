...
export async function request(path, options = {}, { auth = true, retry = true } = {}) {
  ...
  try {
    ...
    // never reached the server at all — offline, DNS failure, the tunnel
    // dropped, etc. Flagged so callers (see src/offline/queue.js) can queue
    // the action for later instead of treating it as a real rejection.
    const networkError = new Error('You appear to be offline.');
