...
export function createGroupStack(name) {
  return request('/groups/create/', { method: 'POST', body: JSON.stringify({ name }) });
}

// `changes.image`, if present, is a browser File object — switches to a
// multipart body, mirroring api/auth.js's uploadAvatar.
export function updateGroupStack(stackId, changes) {
  const { image, ...rest } = changes;
  if (!image) {
    return request(`/groups/${stackId}/`, { method: 'PATCH', body: JSON.stringify(rest) });
  }
  const formData = new FormData();
  if (rest.name !== undefined) formData.append('name', rest.name);
  formData.append('image', image, image.name);
  return request(`/groups/${stackId}/`, { method: 'PATCH', body: formData });
}
